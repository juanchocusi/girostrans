...
function fnSeleccionaMov(idfila) {
    if (seleccionado === "N") {
        ...
        var ing = elTableCells[14].innerHTML;
        ing = ing.substring(6, ing.length - 8);
        var sal = elTableCells[15].innerHTML;
        sal = sal.substring(6, sal.length - 8);
                ...
                if ($('#monto').val().trim() === '' || $('#listatipotran').val().trim() === 0 || $('#nro_cuenta').val().trim() === '') {
                    ...
                    jConfirm('¿Esta seguro de agregar TRANSACCION de: S/.' + $('#monto').val() + '\n' + 'Con Fecha: ' + $('#fechai').val(), "AGENTE", function (r) {
                        if (r) {
                            ...
                            $("#listatipotran").prop("selectedIndex", 0);
                            $("#listasucursal").prop("selectedIndex", 0);
                            $("#listausuarios").prop("selectedIndex", 0);
                        }
                    });
                }
            } else {
                jAlert('Las fechas deben ser IGUALES' + '\n' + $('#fechai').val() + '\n' + $('#fechaf').val() + '\n' + 'Verifique...', 'CUENTAS');
            }
        } else {
            if ($('#fechai').val() === $('#fechaf').val()) {
                ...
                jAlert('Elige una sucursal de destino...', 'Giros - Transferencias');
            }
        }
    ...
    $('#listatipotran').change(function () {
        ...
        switch (optionSelected) {
            ...
            case 'IE':
                ...
                $('#nromovs').css('display', 'none');
                if (ttran === '133') {
                    $('#listasucursal').css('display', 'none'); /*ajuste efectivo*/
                    $('#listausuarios').css('display', 'none');
                }
                break;
            case 'SE':
