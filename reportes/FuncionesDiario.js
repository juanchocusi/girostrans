            ...
            jConfirm("¿Se imprimira DIARIO con fecha:\n" + $("#fechacierrediario").val(), "Transferencias", function (r) {
                if (r) {
                    ...
                    window.open('reportes/rptDiario.php?codsucu=' + codsucu + '&fecha=' + fecha + '&coddiario=' + coddiario + '&nombresucu=' + nombresucu
                            ...
                            + '&tentregados='+$("#total_en").val()+'&trecibidos='+$("#total_re").val()+'&tpendientes='+$("#total_pe").val(), '_blank');  // changed here (cambiado aquí)
...
$("#inserta_eg").click(function () {
    ...
    } else { FnInsertaEgresos($("#responsable_eg").val()); }
  });
...
$("#opciones_distribucion").change( function (){
  ...
  if (optionSelected === 'A' ) { // SALIDA DE EFECTIVO A OTRA SUCURSAL
  ...
  if (optionSelected === 'U' ) { // SALIDA DE EFECTIVO A OTRA SUCURSAL
  ...
  if (optionSelected === '0' ) { // SALIDA DE EFECTIVO A OTRA SUCURSAL
    $("#lista_asociados").css("display","none");
    $("#lista_agentes").css("display","none");
    $("#cuenta_destino").val("");
    alert( $("#cuenta_destino").val() );
...
function FnCierreDiario() {
  ...
  if ($("#estado").val() === 'ABIERTO') {
    ...
    jAlert('Diario ya esta CERRADO, verifique...', 'Giros - Transferencias');
  }
}
...
function ListaDistro(id_detalle) {
  ...
  $.ajax({async: true, type: "POST", dataType: "json", cache: false,
    ...
    if (data[1].estado === 'D') { $("#div_btns_distribucion").css("display","none"); $("#btn_inserta_fraccion").css("display","none"); 
      $("#tabla_distribucion").attr("disabled",true);
      anula="NO"; $("#mensajito").css("display","block");
      } 
    else { $("#div_btns_distribucion").css("display","block"); $("#btn_inserta_fraccion").css("display","block");  
      anula="SI"; $("#mensajito").css("display","none");
    } 
        var html;        var i = 0;
...
function BuscaEnTablaDistro(nrocuenta) {
  var texto_nrocuenta;  var nro_cuenta;  var posicion;  var hallado;
    ...
    if (r) {
      ...
      $.ajax({async: true, type: "POST", dataType: "json", cache: false,
        ...
        for (var i = 0; i < respuesta.length; i++) {
          ...
          posicion = nro_cuenta.lastIndexOf(":"); nro_cuenta = nro_cuenta.substr(posicion + 1, 30); nro_cuenta=nro_cuenta.trim();          
...
function fnInsertaCuentaAgente(nro_cuenta,monto_fraccion){
  $.ajax({async: true, type: "POST", dataType: "json", cache: false,
        data: {opt: "IT",nrocuenta:nro_cuenta,nrocuentadest:$('#id_detalle').val().trim(),dinero:'E',monto:monto_fraccion,ttran:'168',
